...
description: >
  ...
  transform documents. Transformations are handled by yamlify and deyamlify.
---
...
- Produce transformed output (use yamlify / deyamlify)
...
For YAML files:
- **Convention compliance**: Does it have a `meta:` block with `source` or `source_url`? Does it have `intent:` when a downstream agent needs orientation? Flag missing fields only when the file looks like it should be agent-ready.
...
- **Stale markers**: Are any values that appear outdated or superseded? Flag them with a comment — do not add a `# STALE` tag or modify the file directly.
...
- **Mode recommendation**: Given the document's type, length, and downstream use, which mode is appropriate? `lean` for most; `max` for nuance-critical specs; `delta` if updating existing context; `minified` for mid-session reminders.
...
- Are `meta.date` or `meta.source_date` values consistent with the apparent freshness of both files?
...
- **Downstream consumer**: Human reading it → deyamlify. Another agent using
  it as context → yamlify lean with `intent:` and `meta:`. Same agent,
  later in the session → minified.
- **Document type**: Spec with hard constraints → yamlify max (preserves nuance). Notes or scratch → yamlify lean. Architecture doc with nuance → max mode.
...
- Recommendations must name a concrete next action. "Consider optimizing" is
  not a recommendation. "Run yamlify in lean mode on this file" is.
